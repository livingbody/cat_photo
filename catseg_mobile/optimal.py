...
def cat_seg_tracking(pre_gray, cur_gray, prev_cfd, dl_weights, disflow):
    """计算光流跟踪匹配点和光流图
    输入参数:
        pre_gray: 上一帧灰度图
        cur_gray: 当前帧灰度图
...
def cat_seg_track_fuse(track_cfd, dl_cfd, dl_weights, is_track):
    """光流追踪图和人像分割结构融合
    输入参数:
        track_cfd: 光流追踪图
        dl_cfd: 当前帧分割结果
...
def postprocess_v(cur_gray, scoremap, prev_gray, pre_cfd, disflow, is_init):
    ...
    if is_init:
        ...
        track_cfd, is_track, weights = cat_seg_tracking(
            prev_gray, cur_gray, pre_cfd, weights, disflow)
        fusion_cfd = cat_seg_track_fuse(track_cfd, cur_cfd, weights, is_track)
